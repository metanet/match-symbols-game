...
async def _player_inbound(player, game_queue):
    async for message in player.stream:
        try:
            ...
            logging.getLogger(player.name).error("decoding of %s's message: %d failed with: %s", player.name, message, e)
            break
    player.set_inactive()
...
async def _game_loop(randezvous):
    ...
    while True:
        ...
        if message == _START_GAME:
            logger.info(f"Starting the game between %s and %s!" % (randezvous.player1.name, randezvous.player2.name))
            game = game_controller.GameController(_NUM_ROWS, _NUM_COLS, randezvous.player1.name, randezvous.player2.name)
...
async def _start_player(randezvous, player):
...
async def _randezvous_loop(player, client_stream):
    logger = logging.getLogger("randezvous_loop<" + player.name + ">")
    global _randezvous
    ...
    while True:
        ...
        if my_randezvous.is_full():
            ...
            if add_this_player:
                # the players will play against each other for the first time
                await player.enqueue_message(f"You will play with %s.\n" % (opponent.name))
                await opponent.enqueue_message(f"You will play with %s.\n" % (player.name))
                await my_randezvous.game_queue.put(_START_GAME)
                async with my_randezvous.task_group:
                    await _start_player(my_randezvous, player)
            else:
                # the players will play another game togerher
                await player.enqueue_message(f"Starting a new game with %s.\n" % (opponent.name))
                async with my_randezvous.task_group:
                    await _start_player(my_randezvous, player)
                    if player == my_randezvous.player2:
                        await my_randezvous.game_queue.put(_START_GAME)
                        await my_randezvous.task_group.spawn(_game_loop, my_randezvous)
        else:
        ...
        if player.is_active():
            ...
            if opponent.is_active():
                logger.info(f"%s and %s are starting a new game..." % (player.name, opponent.name))
                # game is over. continue with the same randevous since both players are here
                add_this_player = False
            else:
                logger.warning(f"%s has left. %s will wait for a new opponent..." % (opponent.name, player.name))
...
if __name__ == "__main__":
    ...
    print(f"Starting the TCP server on %s:%d for the game deck of %dx%d." % (args.host, args.port, args.rows, args.cols))
